...
def getInput():
    name=nameEntry.get()
    venus = Person(name, datetime.date(int(yearEntry.get()), int(monthEntry.get()), int(dateEntry.get())))
     
    textArea = tk.Text(master=window,height=10,width=25)
    textArea.grid(column=1,row=6)
    answer = " Heyy {venus}!!!. You are {age} years old!!! ".format(venus=name, age=venus.age())
...
image = Image.open('/home/pguser/Downloads/app_image.png' )     #specify the path of the image
